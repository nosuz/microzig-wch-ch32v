                ...
                if (@field(config, field.name)) |pin_config| {
                    ...
                    if (pin_config.function == .GPIO) {
                        ...
                        if (pin_config.usbhd_speed) |speed| {
                            switch (speed) {
                                .Low_speed => {
                                    if ((pin_config.usbhd_buffer_size orelse .byte_8) != .byte_8) @compileError("USB buffer size should be 8 bytes for low-speed.");
                                },
                                .Full_speed => {},
                            }
                        } else {
                            // default speed id low-speed
                            if ((pin_config.usbhd_buffer_size orelse .byte_8) != .byte_8) @compileError("USBHD buffer size should be 8 bytes for low-speed.");
                        }
